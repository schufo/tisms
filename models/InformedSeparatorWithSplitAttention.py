...
class InformedSeparatorWithSplitAttention(nn.Module):
    ...
    def __init__(self, mix_features, mix_encoding_size, mix_encoder_layers, side_info_features,
        ...
        super(InformedSeparatorWithSplitAttention, self).__init__()
...
class AttentionMechanism(nn.Module):
    ...
    def __init__(self, side_info_encoding_size, mix_encoding_size):
        ...
        self.fc = nn.Linear(side_info_encoding_size, side_info_encoding_size)
    ...
    def forward(self, side_info_encoding, mix_encoding):
        ...
        intermediate_score = torch.bmm(self.w_s.expand(batch_size, -1, -1).to(current_device),
                                          torch.transpose(side_info_encoding, 1, 2))

        scores = torch.bmm(mix_encoding, intermediate_score)
        ...
        side_info_encoding_transformed = self.fc(side_info_encoding)

        # compute context vector for each time step of target source decoder
        context = torch.bmm(torch.transpose(side_info_encoding_transformed, 1, 2), torch.transpose(alphas, 1, 2))
