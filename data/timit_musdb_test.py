...
class Test(Dataset):
    ...
    def __init__(self, transform=None):
        ...
        # musdb related
        self.musdb_test_path = os.path.join(path_to_processed_musdb, 'test')

        pickle_in = open(os.path.join(path_to_processed_musdb, 'test/test_file_list.pickle'), 'rb')
        self.test_file_list = pickle.load(pickle_in)

        # make list of shuffled musdb indices to randomly assign a musdb frame to each timit utterance
        musdb_indices_1 = list(np.arange(0, 1487))
        np.random.seed(1)
    ...
    def __len__(self):
        return 1344  # number of utterances in TIMIT test partition without sentences SA1 and SA2

    def __getitem__(self, idx):

        speech, phonemes, phoneme_times, words = get_timit_test_sentence(idx)
        musdb_accompaniment = np.load(os.path.join(self.musdb_test_path,
                                                   self.test_file_list[self.musdb_shuffled_idx[idx]]))
